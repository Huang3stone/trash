...
def checkpoint(model, epoch, opt, updatebest):
...
def main(opt):
    ...
    with open(txt_name, mode='w') as txt_write:
        ...
        # lighten = lowlightnet_ACM(input_dim=3, dim=[16,32,64,128])
        device = torch.device('cuda:' + opt.deviceid)
        lighten.to(device)
        ...
        for epoch in range(opt.start_iter, opt.nEpochs + 1):
            ...
            if epoch % (opt.snapshots) == 0:
                ...
                if max_psnr_score < psnr_score:
                    max_psnr_score = psnr_score
                    updatebest = True
                else:
                    updatebest = False
                print("max_psnr_score:", max_psnr_score)
                file_checkpoint = checkpoint(lighten, epoch, opt, updatebest)
                logs = {
                    "psnr": psnr_score,
                    "ssim": ssim_score,
                }
                ...
                ssim_loss_vaule = ssim_loss.data
                tv_loss_value = tv_loss.data
                percep_loss_value = p_loss.data
                smooth_loss_value = smoothloss
                inner_loss_value = inner_loss
                psnr_value = psnr_score
                ssim_value = ssim_score

                keys = ['loss', 'ssim_loss', 'tv_loss', 'percep_loss', 'smooth_loss', 'inner_loss',  # train loss
                        'psnr', 'ssim']  # params
                # Callback runs at the end of each fit (train+val) epoch
                vals = [loss_value, ssim_loss_vaule, tv_loss_value, percep_loss_value, smooth_loss_value,
                        inner_loss_value
                    , psnr_value, ssim_value]

                x = {k: v for k, v in zip(keys, vals)}  # dict
                filepath = osp.join('model/result', 'result.csv')
                print(filepath)
                n = len(x) + 1  # number of cols
                s = '' if osp.exists(filepath) else (
