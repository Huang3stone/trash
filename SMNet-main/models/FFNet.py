...
from research.common import illumination_module, space_to_depth, C3, SPPF, C3_Res2Block, SAMBlock, illumination_module2
from research.attention import GAM_Attention, CBAMBlock, SimAM, SEAttention, NAMAttention, EffectiveSEModule, \
    ParNetAttention, TripletAttention, ShuffleAttention, CoordAtt, ParallelAttention
...
class lowlightnet_ACM(nn.Module):
    def __init__(self, input_dim=3, dim=[16, 32, 64, 128]):
        ...
        for i in range(len(dim) - 1, 0, -1):
            stage = nn.Sequential(
                nn.Conv2d(dim[i], dim[i], 1, stride=1, padding=0, bias=False),
                nn.PReLU(),
                nn.ConvTranspose2d(dim[i], dim[i], 3, stride=2, padding=1, output_padding=1, bias=False),
                nn.PReLU(),
                nn.Conv2d(dim[i], dim[i] // 2, 1, stride=1, padding=0, bias=False),
            )
